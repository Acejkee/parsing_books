...
def get_data():
    ...
    for page in range(1, pages_count + 1):
        ...
        for bi in books_items:
            ...
            try:
                book_new_price = int(
                    book_data[3].find("div", class_="price").find("span").find("span").text.replace(" ",
                                                                                                    ""))
            except:
                book_new_price = "Нет нового прайса"

            try:
                book_old_price = int(book_data[3].find("span", class_="price-old").text.replace(" ", ""))
            except:
...
def main():
    ...
    print(f"На скрипт было затрачено {end_time}")
